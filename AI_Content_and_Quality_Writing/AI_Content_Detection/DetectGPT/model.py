...
class GPT2PPLV2(torch.nn.Module):
    ...
    def __call__(self, **kwargs):
        ...
        if version == "v1.1":
            return self.call_1_1(sentence, kwargs["chunk_value"])
        elif version == "v1":
...
###############################################
    ...
    def call_1_1(self, sentences, chunk_value):
        output_labels = torch.tensor([], device=self.device, dtype=torch.float)
        for sentence in sentences:
            ...
            label = torch.tensor([[1.0 if mean_score > self.threshold else 0.0]], device=self.device, dtype=torch.float)
            output_labels = torch.cat([output_labels, label], dim=0)
        print(output_labels)
        return { "logits": output_labels, "loss": torch.zeros_like(output_labels, device=self.device, dtype=torch.float) }
